...
<body>
    ...
    <input type="search" name="search" class="input-text header-searchbar" placeholder="Search Schedule Date or Time" list="schedules">&nbsp;&nbsp;
    <?php
        echo '<datalist id="schedules">';
        $list11 = $database->query("SELECT DISTINCT scheduledate, scheduletime FROM schedule;");

        for ($y = 0; $y < $list11->num_rows; $y++) {
            $row00 = $list11->fetch_assoc();
            $date = $row00["scheduledate"];
            $time = $row00["scheduletime"];
            echo "<option value='$date'><br/>";
            echo "<option value='$time'><br/>";
        }
        echo ' </datalist>';
    ?>
    <input type="Submit" value="Search" class="login-btn btn-primary btn" style="padding-left: 25px;padding-right: 25px;padding-top: 10px;padding-bottom: 10px;">
                ...
                <?php
                    ...
                    if ($_POST) {
                        ...
                        // Search by schedule titles, dates, and times
                        $sqlmain = "SELECT DISTINCT sc.*, d.docname
                                    FROM schedule sc
                                    LEFT JOIN doctor d ON sc.docid = d.docid
                                    LEFT JOIN specialties s ON d.specialties = s.id
                                    WHERE sc.title LIKE '%$keyword%'
                                        OR sc.scheduledate = '$keyword'
                                        OR sc.scheduletime = '$keyword'
                                    ORDER BY sc.scheduleid DESC";
                    } else {
                        // Show all schedules if no search keyword is provided
                        $sqlmain = "SELECT sc.*, d.docname
                                    FROM schedule sc
                                    LEFT JOIN doctor d ON sc.docid = d.docid
                                    ORDER BY sc.scheduleid DESC";
                    }
