...
<body>
    ...
    <input type="search" name="search" class="input-text header-searchbar" placeholder="Search Doctor name, Email, or Specialties" list="doctors">&nbsp;&nbsp;
    <?php
        echo '<datalist id="doctors">';
        $list11 = $database->query("SELECT docname, docemail, specialties FROM doctor INNER JOIN specialties ON doctor.specialties = specialties.id;");

        for ($y = 0; $y < $list11->num_rows; $y++) {
            $row00 = $list11->fetch_assoc();
            $d = $row00["docname"];
            $c = $row00["docemail"];
            $s = $row00["sname"]; // Fetch the specialty name from the joined table
            echo "<option value='$d'><br/>";
            echo "<option value='$c'><br/>";
            echo "<option value='$s'><br/>";
        };
        echo ' </datalist>';
    ?>
    ...
    if($_POST){
        ...
        // Search by doctor's email, name, or specialties
        $sqlmain = "SELECT DISTINCT d.*
                    FROM doctor d
                    LEFT JOIN specialties s ON d.specialties = s.id
                    WHERE d.docemail = '$keyword'
                        OR d.docname = '$keyword'
        ...
        // Show all doctors if no search keyword is provided
